...
def build_exercise(raw, where):
    ...
    block["check"] = derive_check(ex, where)
    return block


CHECK_KINDS = {"stdoutEquals", "testsPass", "typecheckPasses", "commandsRun", "self"}


def derive_check(ex, where):
    """How the platform can grade this exercise automatically.

    Explicit `check:` in the YAML wins; otherwise:
      code exercise with expectedOutput      -> stdoutEquals  (run `run`, compare stdout)
      code exercise on a *.spec.ts file      -> testsPass     (run `run`, all tests must pass)
      other code exercise (e.g. config edit) -> typecheckPasses
      terminal exercise                      -> commandsRun   (each solution command exits 0)
      written / predict                      -> self          (learner compares with the model answer)
    """
    explicit = ex.get("check")
    if explicit:
        if isinstance(explicit, str):
            explicit = {"kind": explicit}
        if explicit.get("kind") not in CHECK_KINDS:
            raise ContentError(f"{where}: unknown check kind {explicit.get('kind')}")
        return explicit
    t = ex["type"]
    cwd = "pw-course/ts-basics" if str(ex.get("file", "")).startswith("ts-basics/") else "pw-course"
    if t == "code" and ex.get("expectedOutput"):
        return {"kind": "stdoutEquals", "cwd": cwd, "run": ex.get("run"), "expected": str(ex["expectedOutput"]).rstrip("\n")}
    if t == "code" and str(ex.get("file", "")).endswith(".spec.ts"):
        return {"kind": "testsPass", "cwd": cwd, "run": ex.get("run")}
    if t == "code":
        return {"kind": "typecheckPasses", "file": ex.get("file")}
    if t == "terminal":
        cmds = [c for c in str(ex.get("solution", "")).splitlines() if c.strip() and not c.strip().startswith("#")]
        return {"kind": "commandsRun", "commands": cmds}
    return {"kind": "self"}
    ...
    while i < len(body):
        ...
        if fm:
            ...
            if lang == "quiz":
                ...
                # collapsible reference material (Markdown inside), shown behind a toggle
                add_block({"type": "reference", "title": attrs.get("title", "Reference"), "content": raw})
            elif lang == "output":
...
def main():
    ...
    weeks = {}
    for d in days:
        weeks.setdefault(d.get("week", 1), []).append(d)
    course = {
        "schemaVersion": "1.0",
        "title": "Playwright with TypeScript — Fundamentals, Setup & Programming",
        "weeks": [{"week": w, "title": WEEK_TITLES.get(w, f"Week {w}"), "days": ds} for w, ds in sorted(weeks.items())],
    }
    (JSON_DIR / "course.json").write_text(json.dumps(course, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    for w, ds in sorted(weeks.items()):
        (JSON_DIR / f"week{w}.json").write_text(json.dumps(
            {"schemaVersion": "1.0", "week": w, "title": WEEK_TITLES.get(w, f"Week {w}"), "days": ds},
            indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
        print(f"✓ week{w}.json ({len(ds)} days)")
    print(f"✓ course.json ({len(days)} days)")
